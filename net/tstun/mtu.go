...
// TunMTU is used to describe MTUs for the tailscale TUN.
type TunMTU uint32

// WireMTU is used to describe MTUs for the underlying network devices.
type WireMTU uint32

const (
	// Largest MTU we will consider for the Tailscale TUN. This is inherited
	// from wireguard-go and can be surprisingly small; on Windows it is
	// currently 2048 - 32 bytes.
...
//
// XXX These need adjusting before merging to main. What do you think?
...
func init() {
	for _, m := range WireMTUsToProbe {
		// Remove MTUs larger than the device max MTU.
		if m > WireMTU(maxTunMTU) {
			// TODO: actually remove these from the list to probe.
			continue
		}
